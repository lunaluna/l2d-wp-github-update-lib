 ...
 *
 * 契約(初版で凍結。後から変更しない):
 * 1. l2d_updater_register( $version, $class_file, array $config ) の引数を増やさない.
 *    拡張は $config のキー追加で行う.
...
if ( ! function_exists( 'l2d_updater_register' ) ) {
	 ...
	 */
	function l2d_updater_register( $version, $class_file, array $config ) {
		global $l2d_updater_registry;
		if ( ! is_array( $l2d_updater_registry ) ) {
			$l2d_updater_registry = array(
				'files'   => array(),
				'configs' => array(),
			);
		}
		$l2d_updater_registry['files'][ (string) $version ] = $class_file;
		$l2d_updater_registry['configs'][]                  = $config;
	}
	 ...
	 */
	function l2d_updater_boot() {
		global $l2d_updater_registry;
		if ( empty( $l2d_updater_registry['files'] ) ) {
			return;
		}

		$files = $l2d_updater_registry['files'];
		uksort( $files, 'version_compare' );
		$winner = end( $files );

		require_once $winner;

		foreach ( $l2d_updater_registry['configs'] as $config ) {
			new L2d_GitHub_Updater( $config );
		}
	}

	add_action( 'plugins_loaded', 'l2d_updater_boot', -100 );
}

/**
 * このコピー固有のバージョンとファイルパスをクロージャでキャプチャして返す.
 *
 * グローバル定数(例: L2D_UPDATER_LIB_VERSION)を使わない理由: 複数プラグインが
...
return function ( array $config ) {
	l2d_updater_register( '1.0.0', __DIR__ . '/class-l2d-github-updater.php', $config );
};
