...
class LoaderTest extends PHPUnit\Framework\TestCase {
	...
	protected function setUp(): void {
		parent::setUp();
		$GLOBALS['l2d_updater_registry']         = null;
	 ...
	 * ファイルパスで l2d_updater_register() が呼ばれること.
	 */
	public function test_returned_closure_registers_this_copy() {
		...
		$this->assertSame(
			...
			array_values( $l2d_updater_registry['files'] )
		);
		$this->assertSame( array( array( 'slug' => 'plugin-a' ) ), $l2d_updater_registry['configs'] );
	}
	...
	public function test_boot_uses_highest_version() {
		...
		l2d_updater_register( '1.0.0', __DIR__ . '/fixtures/fake-class-v1.php', array( 'slug' => 'plugin-a' ) );
		l2d_updater_register( '2.0.0', __DIR__ . '/fixtures/fake-class-v2.php', array( 'slug' => 'plugin-b' ) );
		l2d_updater_register( '1.5.0', __DIR__ . '/fixtures/fake-class-v1.php', array( 'slug' => 'plugin-c' ) );
	...
	/**
	 * 何も登録されていない状態で l2d_updater_boot() を呼んでもエラーにならず、
	 * 何も require されないこと.
	 */
	...
	/**
	 * l2d_updater_register() は $config を検証も加工もせず素通しすること.
	 */
	public function test_register_passes_config_through_without_modification() {
		...
		l2d_updater_register( '1.0.0', __DIR__ . '/fixtures/fake-class-v1.php', $config );

		global $l2d_updater_registry;

		$this->assertSame( array( $config ), $l2d_updater_registry['configs'] );
	}
}
